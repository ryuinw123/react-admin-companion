  ...
  const handleFilter = (event) => {
    ...
    const newFilter = data.filter((value) => {
      return (
        ...
        value.created_user?.toString().toLowerCase().includes(searchWord.toLowerCase()) ||
        value.event_id?.toString().toLowerCase().includes(searchWord.toLowerCase()) ||
        value.eventname?.toLowerCase().includes(searchWord.toLowerCase()) ||
        value.student?.toString().toLowerCase().includes(searchWord.toLowerCase())
      );
    });
      ...
      {filteredData.length != 0 && (
        <div className="dataResult">
          {filteredData.slice(0, 15).map((value, key) => {
            return (
              <a className="dataItem" onClick={(e) => {
                e.preventDefault()
                onSearchBarClick(value)
                }} target="_blank" key={uuidv4()}>
